...
Pour chaque item, le nœud SHALL pouvoir afficher optionnellement **titre**, **description**, **compteur** et **like**, contrôlés par `content.show.title`, `content.show.description`, `content.show.counter` et `content.show.like`. Le nœud SHALL **ne pas** afficher d’image, y compris via le champ `image` du mapping ApiCard ou des balises `<img>` dans le contenu HTML. Lorsqu’un toggle `show` est activé mais que le champ correspondant est absent dans l’item mappé, le nœud SHALL omettre cet élément sans réserver d’espace vide. Lorsqu’un toggle `show` est désactivé, le nœud SHALL ne pas rendre cet élément quel que soit le contenu mappé.
