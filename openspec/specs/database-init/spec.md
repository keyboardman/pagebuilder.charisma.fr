# database-init Specification

## Purpose
TBD - created by archiving change initialise-database. Update Purpose after archive.
## Requirements
...
- **WHEN** `APP_ENV=test` est défini, `DATABASE_URL` (ou le suffix) pointe vers une base de test et `symfony console doctrine:database:create` puis `symfony console doctrine:migrations:migrate --no-interaction` sont exécutés
...
La procédure d’initialisation de la base (démarrage de PostgreSQL, `symfony console doctrine:database:create`, `symfony console doctrine:migrations:migrate`) et la dépendance à `DATABASE_URL` SHALL être documentée dans `openspec/project.md` ou un README à la racine du projet.

#### Scenario: Démarrage depuis zéro

- **WHEN** un développeur suit la procédure documentée (PostgreSQL démarré, `DATABASE_URL` défini, `symfony console doctrine:database:create`, `symfony console doctrine:migrations:migrate`)
