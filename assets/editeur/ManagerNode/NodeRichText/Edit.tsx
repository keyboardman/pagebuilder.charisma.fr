...
import { $createCodeNode, CodeNode, $isCodeNode } from "@lexical/code";
import {
  CAN_REDO_COMMAND,
  CAN_UNDO_COMMAND,
...
const Edit: FC<NodeEditProps> = () => {
  ...
  const initialConfig = useMemo(
    () => ({
      namespace: `node-rich-text-${node.id}`,
  ...
  return (
    <div
      ...
      data-ce-type={node.type}
      id={node?.attributes?.id ?? ""}
      className={cn("ce-rich-text ce-rich-text-pg editor-shell", node?.attributes?.className ?? "")}
      style={viewStyle}
    >
