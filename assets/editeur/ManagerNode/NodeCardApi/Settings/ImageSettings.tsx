...
import { ClassName2Settings, getCardApiThemeSelector } from "../../Settings";
import { Border2Settings, Object2Settings, Background2Settings } from "../../Settings";

export function ImageSettings() {
  const { node, onChange } = useNodeBuilderContext();
  const cardNode = node as NodeType;
  const themeOverrideSelector = getCardApiThemeSelector(cardNode.content?.container?.position, "image");
  ...
  return (
    <div className="flex flex-1 flex-col gap-1">
      ...
      />
      <Object2Settings themeOverrideSelector={themeOverrideSelector}
        style={style} onChange={(style) => onChange({
        ...node,
        content: {
          ...cardNode.content,
