...
import {
  fetchDynamicListCollectionCached,
  fetchDynamicListSources,
  type ListApiMappedItem,
} from "./listApiUtils";

interface ListApiItemPickerModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSelect: (sourceId: string, item: ListApiMappedItem) => void;
}

export function ListApiItemPickerModal({ open, onOpenChange, onSelect }: ListApiItemPickerModalProps) {
  ...
  useEffect(() => {
    ...
    const loadItems = async () => {
      ...
      try {
        const response = await fetchDynamicListCollectionCached(
          selectedSourceId,
          page,
          20,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="api-manager-ui flex max-h-[80vh] max-w-2xl flex-col">
        <DialogHeader>
          <DialogTitle className="node-block-title">Sélectionner un article</DialogTitle>
          <DialogDescription>
            Choisissez une source ApiListArticleDynamique puis un article à composer dans la liste.
          </DialogDescription>
        </DialogHeader>

        <div className="flex flex-1 flex-col gap-4 overflow-hidden">
          {listOptions.length === 0 ? (
            <p className="text-center text-sm text-muted-foreground">Aucune source dynamique disponible.</p>
          ) : (
            <>
              <Form.Group>
              ...
              {!loading && !error && selectedSourceId && items.length === 0 ? (
                <p className="text-center text-sm text-muted-foreground">Aucun article trouvé.</p>
              ) : null}
                ...
                <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
                  <div className="min-h-0 flex-1 space-y-2 overflow-y-auto">
                    {items.map((item) => (
                      ...
                      >
                        <p className="truncate text-sm font-medium">{item.title || "Sans titre"}</p>
                        {item.description ? (
...
export default ListApiItemPickerModal;
