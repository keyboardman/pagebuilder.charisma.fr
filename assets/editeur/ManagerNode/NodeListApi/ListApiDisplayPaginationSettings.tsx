...
import { DEFAULT_LIST_API_ITEMS_PER_PAGE, MAX_LIST_API_ITEMS_PER_PAGE } from "./listApiUtils";

interface ListApiDisplayPaginationSettingsProps {
  page?: number;
  itemsPerPage?: number;
  totalPages?: number;
...
export function ListApiDisplayPaginationSettings({
  page,
  itemsPerPage,
  totalPages = 0,
  onPageChange,
  onItemsPerPageChange,
}: ListApiDisplayPaginationSettingsProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-x-4 gap-y-2">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <Form.Input
          type="number"
          value={String(itemsPerPage ?? DEFAULT_LIST_API_ITEMS_PER_PAGE)}
          onChange={(value: string) => {
            const num = parseInt(value, 10);
            if (Number.isNaN(num) || num < 1) {
              return;
            ...
            onItemsPerPageChange(Math.min(num, MAX_LIST_API_ITEMS_PER_PAGE));
          }}
          className="h-7 w-12 shrink-0 px-1 text-center text-[0.75rem]"
          min={1}
          max={MAX_LIST_API_ITEMS_PER_PAGE}
        />
      </div>
    </div>
  );
