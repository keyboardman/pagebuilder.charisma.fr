...
import { COLLECTION_IMAGE_MEDIA_TYPE, type CollectionImageMediaEntry } from "../index";
...
interface CollectionImageDynamicItemsSettingsProps {
  items: CollectionImageMediaEntry[];
  onChange: (items: CollectionImageMediaEntry[]) => void;
}

export const CollectionImageDynamicItemsSettings: FC<CollectionImageDynamicItemsSettingsProps> = ({
  items,
  onChange,
}) => {
  ...
  const handleLinkChange = (index: number, link: string) => {
    onChange(
      items.map((entry, idx) => (idx === index ? { ...entry, link } : entry))
    );
  };
          ...
          {items.map((entry, index) => {
            ...
            return (
              <li
                ...
                className={cn(
                  "space-y-1.5 rounded border border-border/60 bg-background px-2 py-1.5",
...
export default CollectionImageDynamicItemsSettings;
