...
export function resolveColorValue (value: string): string {
    ...
    const darkenMatch = trimmed.match(
        /^darken\s*\(\s*(.+?)\s*,\s*(\d+(?:\.\d+)?)\s*%?\s*\)\s*$/i
    )
    if (darkenMatch) {
    ...
    const lightenMatch = trimmed.match(
        /^lighten\s*\(\s*(.+?)\s*,\s*(\d+(?:\.\d+)?)\s*%?\s*\)\s*$/i
    )
    if (lightenMatch) {
...
export const NODE_OVERRIDE_TARGETS = [
    {
        key: 'node-container',
        label: 'NodeContainer',
        selector: '.ce-container'
    },
    { key: 'node-flex', label: 'NodeFlex', selector: '.ce-flex' },
    { key: 'node-grid', label: 'NodeGrid', selector: '.ce-grid' },
    { key: 'node-hero', label: 'NodeHero', selector: '.ce-hero' },
    {
        key: 'node-two-columns',
        label: 'NodeTwoColumns',
        selector: '.ce-two_columns'
    },
    ...
    { key: 'node-html', label: 'NodeHtml', selector: '.ce-html' },
    { key: 'node-header', label: 'NodeHeader', selector: '.ce-header' },
    {
        key: 'node-header-h1',
        label: 'NodeHeader H1',
        selector: '.ce-header-h1',
        tag: 'h1'
    },
    {
        key: 'node-header-h2',
        label: 'NodeHeader H2',
        selector: '.ce-header-h2',
        tag: 'h2'
    },
    {
        key: 'node-header-h3',
        label: 'NodeHeader H3',
        selector: '.ce-header-h3',
        tag: 'h3'
    },
    {
        key: 'node-header-h4',
        label: 'NodeHeader H4',
        selector: '.ce-header-h4',
        tag: 'h4'
    },
    {
        key: 'node-header-h5',
        label: 'NodeHeader H5',
        selector: '.ce-header-h5',
        tag: 'h5'
    },
    {
        key: 'node-header-h6',
        label: 'NodeHeader H6',
        selector: '.ce-header-h6',
        tag: 'h6'
    },
    { key: 'node-image', label: 'NodeImage', selector: '.ce-image' },
    {
        key: 'node-slideshow',
        label: 'NodeSlideshow',
        selector: '.ce-slideshow'
    },
    { key: 'node-video', label: 'NodeVideo', selector: '.ce-video' },
    {
        key: 'node-video-icon-player-inner',
        label: 'NodeVideoIconPlayerInner',
        selector: '.ce-video-icon-player-inner'
    },
    { key: 'node-youtube', label: 'NodeYoutube', selector: '.ce-youtube' },
    { key: 'node-button', label: 'NodeButton', selector: '.ce-button' },
    {
        key: 'node-top-button',
        label: 'NodeTopButton',
        selector: '.ce-top-button'
    },
    { key: 'node-card', label: 'NodeCard', selector: '.ce-card' },
    {
        key: 'node-card-top-image',
        label: 'NodeCardTopImage',
        selector: '.ce-card-position-top .ce-card-image'
    },
    {
        key: 'node-card-top-title',
        label: 'NodeCardTopTitle',
        selector: '.ce-card-position-top .ce-card-title'
    },
    {
        key: 'node-card-top-text',
        label: 'NodeCardTopText',
        selector: '.ce-card-position-top .ce-card-text'
    },
    {
        key: 'node-card-top-label',
        label: 'NodeCardTopLabel',
        selector: '.ce-card-position-top .ce-card-label'
    },
    {
        key: 'node-card-left-image',
        label: 'NodeCardLeftImage',
        selector: '.ce-card-position-left .ce-card-image'
    },
    {
        key: 'node-card-left-title',
        label: 'NodeCardLeftTitle',
        selector: '.ce-card-position-left .ce-card-title'
    },
    {
        key: 'node-card-left-text',
        label: 'NodeCardLeftText',
        selector: '.ce-card-position-left .ce-card-text'
    },
    {
        key: 'node-card-left-label',
        label: 'NodeCardLeftLabel',
        selector: '.ce-card-position-left .ce-card-label'
    },
    {
        key: 'node-card-right-image',
        label: 'NodeCardRightImage',
        selector: '.ce-card-position-right .ce-card-image'
    },
    {
        key: 'node-card-right-title',
        label: 'NodeCardRightTitle',
        selector: '.ce-card-position-right .ce-card-title'
    },
    {
        key: 'node-card-right-text',
        label: 'NodeCardRightText',
        selector: '.ce-card-position-right .ce-card-text'
    },
    {
        key: 'node-card-right-label',
        label: 'NodeCardRightLabel',
        selector: '.ce-card-position-right .ce-card-label'
    },
    {
        key: 'node-anniversaire',
        label: 'NodeAnniversaire',
        selector: '.ce-anniversaire'
    },
    {
        key: 'node-video-home',
        label: 'NodeVideoHome',
        selector: '.ce-video-home'
    },
    {
        key: 'node-puremusic-top-semaine',
        label: 'NodePureMusicTopSemaine',
        selector: '.ce-puremusic-top-semaine'
    },
    {
        key: 'node-card-video',
        label: 'NodeCardVideo',
        selector: '.ce-card-video'
    },
    { key: 'node-card-api', label: 'NodeCardApi', selector: '.ce-card-api' }
] as const

export const HEADER_OVERRIDE_TARGETS = NODE_OVERRIDE_TARGETS.filter(target =>
    target.key.startsWith('node-header-h')
)

export function buildInitialNodeOverrides (
    initial: Record<string, string> | null | undefined
): Record<string, string> {
    const out: Record<string, string> = {}
    for (const target of NODE_OVERRIDE_TARGETS) {
        out[target.key] = String(initial?.[target.key] ?? '')
    }
    return out
}
...
export const HEADING_AND_P_BLOCKS = [
    'h1',
    'h2',
    'h3',
    'h4',
...
export const BUTTON_VARIANT_LABELS: Record<
    typeof BUTTON_VARIANT_KEYS[number],
    string
> = {
    ch_btn: 'Default',
...
export const BUTTON_VARIANT_COLOR_KEYS = [
    'background',
    'color',
    'border-color'
] as const
...
export function blockToCamelCase (
...
export function rawToButtonConfig (
...
export function rawToButtonSizeConfig (
    raw: Record<string, string> | null | undefined
): ButtonSizeConfig {
...
export function rawToButtonColorOnly (
    raw: Record<string, string> | null | undefined
): ButtonColorOnly {
    if (!raw || typeof raw !== 'object')
        return { ...BUTTON_COLOR_ONLY_DEFAULTS }
...
export function buttonConfigToStyle (b: ButtonConfig): React.CSSProperties {
    ...
    if (b.fontFamily)
        s.fontFamily = b.fontFamily.includes(',')
            ? b.fontFamily
            : `"${b.fontFamily}", sans-serif`
    return s
}

export function mergeButtonWithSize (
    base: ButtonConfig,
...
export function getVariantButtonStyle (
    base: ButtonConfig,
    ...
    if (variant.background)
        overrides.background = resolveColorValue(variant.background)
    if (variant.color) overrides.color = resolveColorValue(variant.color)
    if (variant.borderColor?.trim())
        overrides.borderColor = resolveColorValue(variant.borderColor.trim())
...
export function getStateButtonStyle (
    ...
    if (stateColors.background)
        overrides.background = resolveColorValue(stateColors.background)
    if (stateColors.color)
        overrides.color = resolveColorValue(stateColors.color)
    if (stateColors.borderColor?.trim())
        overrides.borderColor = resolveColorValue(
            stateColors.borderColor.trim()
        )
    return { ...baseStyle, ...overrides }
}

export function configToSelectedFontValues (
    config: ThemeConfigJson | null | undefined,
    fonts: { id: number; name: string }[]
): string[] {
...
export function blockToStyle (block: BodyConfig): React.CSSProperties {
    ...
    if (block.fontFamily)
        style.fontFamily = block.fontFamily.includes(',')
            ? block.fontFamily
...
export function mapToCssString (map: Record<string, string>): string {
...
export function stringCssToMap (raw: string): Record<string, string> {
    ...
    const withoutBlockMap = clean
        ...
        .replace(/\}\s*$/, '')
        .trim()
    for (const chunk of withoutBlockMap.split(';')) {
...
export function toReactStyle (map: Record<string, string>): React.CSSProperties {
    const textAlign = (map['text-align'] ?? '').trim()
    const fontWeight = (map['font-weight'] ?? '').trim()

    return {
        aspectRatio: map['aspect-ratio'] ?? '',
        backgroundColor: map['background-color'] ?? '',
        backgroundImage: map['background-image'] ?? '',
        borderColor: map['border-color'] ?? '',
        borderWidth: map['border-width'] ?? '',
        borderStyle: map['border-style'] ?? '',
        borderRadius: map['border-radius'] ?? '',
        color: map['color'] ?? '',
        fontFamily: map['font-family'] ?? '',
        fontSize: map['font-size'] ?? '',
        fontWeight: fontWeight || undefined,
        gap: map['gap'] ?? '',
        WebkitLineClamp: map['line-clamp'] ?? '',
