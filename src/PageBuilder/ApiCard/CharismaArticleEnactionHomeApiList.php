...
/**
 * Liste « En Action » (home) depuis https://api.charisma.fr/api/charisma/article/enactions/home
 *
 ...
 * - url → link
 */
final class CharismaArticleEnactionHomeApiList extends AbstractApiCardList implements ApiCardBehaviorInterface
{
    ...
    public function fetchCollection(array $params): array
    {
        ...
        try {
            $response = $this->httpClient->request('GET', self::BASE_URL . '/api/charisma/article/enactions/home', [
                'query' => $query,
                'timeout' => 30,
            ]);
    ...
    public function fetchItem(string $id): object
    {
        try {
            $response = $this->httpClient->request(
                'GET',
                self::BASE_URL . '/api/charisma/article/enactions/' . rawurlencode($id),
                ['timeout' => 30]
            );
