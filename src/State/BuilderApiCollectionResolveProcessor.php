...
/**
 * @implements ProcessorInterface<null, BuilderApiCollectionResolveResponse>
 */
final class BuilderApiCollectionResolveProcessor implements ProcessorInterface
{
    public function __construct(
        private readonly ApiCollectionResolver $resolver,
        private readonly BuilderApiResourceFactory $resourceFactory,
    ) {
    }

    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): BuilderApiCollectionResolveResponse
    {
        ...
        foreach ($rawEntries as $rawEntry) {
            ...
            $entry = ApiCollectionResolveEntry::fromArray($rawEntry);
            if ($entry !== null) {
                $entries[] = $entry;
            }
        }

        $response = new BuilderApiCollectionResolveResponse();
        foreach ($this->resolver->resolve($entries) as $itemData) {
            $response->items[] = $this->resourceFactory->createCollectionItemData($itemData);
        }
